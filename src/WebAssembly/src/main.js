...
function detectFace() {
  ...
  startAsmWorker(
    asmCanvas.real.context.getImageData(
      0,
      0,
      asmCanvas.real.canvas.width || 200,
      asmCanvas.real.canvas.height || 200
    ),
    'faceDetect'
  );
  perfwasm0 = performance.now();
  startWasmWorker(
    wasmCanvas.real.context.getImageData(
      0,
      0,
      wasmCanvas.real.canvas.width || 200,
      wasmCanvas.real.canvas.height || 200
    ),
    'faceDetect'
  );
  perfJS0 = performance.now();
  startJSWorker(
    jsCanvas.real.context.getImageData(
      0,
      0,
      jsCanvas.real.canvas.width || 200,
      jsCanvas.real.canvas.height || 200
    ),
    'faceDetect'
  );
}
...
function startWasmWorker(imageData, command) {
  wasmCanvas.holder.context.drawImage(
    img,
    0,
    0,
    imageData.width,
  ...
  let message = {
    cmd: command,
    img: wasmCanvas.holder.context.getImageData(
      0,
      0,
      imageData.width,
      imageData.height
...
function startAsmWorker(imageData, command) {
  asmCanvas.holder.context.drawImage(
    img,
    0,
    0,
    imageData.width,
  ...
  let message = {
    cmd: command,
    img: asmCanvas.holder.context.getImageData(
      0,
      0,
      imageData.width,
      imageData.height
...
function startJSWorker(imageData, command) {
  jsCanvas.holder.context.drawImage(
    img,
    0,
    0,
    imageData.width,
  ...
  let message = {
    cmd: command,
    img: jsCanvas.holder.context.getImageData(
      0,
      0,
      imageData.width,
      imageData.height
...
function updateCanvas(e, canvas) {
  ...
  for (let i = 0; i < e.data.features.length; i++) {
    ...
    canvas.real.context.strokeRect(
      rect.x * canvas.scale,
      rect.y * canvas.scale,
...
wasmWorker.onmessage = function(e) {
  ...
  if (!wasmRan) {
    ...
    document.getElementById('wasm-speed').innerText =
      String((perfwasm1 - perfwasm0).toFixed(2)) + ' MS';
  }
};
...
asmWorker.onmessage = function(e) {
  ...
  if (!asmRan) {
    ...
    document.getElementById('asm-speed').innerText =
      String((perfasm1 - perfasm0).toFixed(2)) + ' MS';
  }
};
...
jsWorker.onmessage = function(e) {
  ...
  if (!jsRan) {
    ...
    document.getElementById('js-speed').innerText =
      String((perfjs1 - perfJS0).toFixed(2)) + ' MS';
  }
};
...
function handleFiles(e) {
  ...
  img.onload = function() {
    wasmCanvas.real.canvas.width = asmCanvas.real.canvas.width = jsCanvas.real.canvas.width = wasmCanvas.holder.canvas.width = asmCanvas.holder.canvas.width = jsCanvas.holder.canvas.width =
      img.width;

    wasmCanvas.real.canvas.height = asmCanvas.real.canvas.height = jsCanvas.real.canvas.height = wasmCanvas.holder.canvas.height = asmCanvas.holder.canvas.height = jsCanvas.holder.canvas.height =
      img.height;
