  ...
  navigator.getUserMedia(
    {
      ...
      video: true
      //   video: {
      //     width: canvases.width * canvases.scale,
      //     height: canvases.height * canvases.scale
      //   }
    },
    handleVideo,
...
function detect(type) {
  if (!canvases.running) {
    ...
    startWorker(
      canvases.wasm.context.getImageData(
        0,
        0,
        canvases.wasm.canvas.width,
        canvases.wasm.canvas.height
      ),
      objType,
      'wasm'
    );
    startWorker(
      canvases.asm.context.getImageData(
        0,
        0,
        canvases.asm.canvas.width,
        canvases.asm.canvas.height
      ),
      objType,
      'asm'
    );
    // startWorker(
    //   canvases.js.context.getImageData(
    //     0,
    //     0,
    //     canvases.js.canvas.width,
    //     canvases.js.canvas.height
    //   ),
    //   objType,
    //   'js'
    // );
...
function startWorker(imageData, command, type) {
  ...
  if (type == 'wasm')
    canvases.dummy.context.drawImage(
      wasm,
      0,
      0,
      imageData.width,
      ...
      0,
      0,
      Math.round(imageData.width / canvases.scale),
...
function updateCanvas(e, targetCanvas, plot) {
  ...
  if (
    // canvases.js.fpsArr.length === 1 ||
    canvases.asm.fpsArr.length === 2 ||
    canvases.wasm.fpsArr.length === 4
  ) {
    targetCanvas.context.fps =
      Math.round(
        (targetCanvas.fpsArr.reduce((a, b) => a + b) /
          targetCanvas.fpsArr.length) *
          100
      ) / 100;
    if (
      targetCanvas.context.fps >
      myChart.controller.options.scales.yAxes[0].ticks.max
    ) {
      myChart.controller.options.scales.yAxes[0].ticks.max =
        targetCanvas.context.fps;
    }
  ...
  for (let i = 0; i < e.data.features.length; i++) {
    ...
    targetCanvas.context.strokeRect(
      rect.x * canvases.scale,
      rect.y * canvases.scale,
      rect.width * canvases.scale,
      rect.height * canvases.scale
    );
  }
}
...
let wasmGraph = {
  displayPoints: myChart.config.data.datasets[0].data,
  holder: []
};
let asmGraph = {
  displayPoints: myChart.config.data.datasets[1].data,
  holder: []
};
// let jsGraph = {
//   displayPoints: myChart.config.data.datasets[2].data,
//   holder: []
// };
